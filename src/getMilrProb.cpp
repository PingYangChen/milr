...
struct Worker_getMilrProb: public RcppParallel::Worker {
  ...
  Worker_getMilrProb(const uvec& bag2, const mat& X, const vec& beta, vec& prob):
    bag2(bag2), X(X), beta(beta), prob(prob) {}
...
arma::vec getMilrProb(const arma::vec& beta, const arma::mat& X, const arma::vec& bag) {
  ...
  Worker_getMilrProb getMilrProb_worker(bag2, X, beta, prob);
